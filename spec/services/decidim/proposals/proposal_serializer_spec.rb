    ...
    describe ProposalSerializer do
      ...
      describe "#serialize" do
        ...
        it "serializes the author ID" do
          expect(serialized).to include(author_id: proposal.creator_identity.id)
        end

        context "when it is a official proposal" do
          let!(:proposal) { create(:proposal, :accepted, :official) }

          it "serializes the author ID with 'official'" do
            expect(serialized).to include(author_id: "Official")
          end
        end
